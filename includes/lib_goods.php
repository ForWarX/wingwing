...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, g.promote_end_date, g.promote_price, SUM(og.goods_number) as goods_number ' .
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
		...
		$arr[$i]['short_name'] = remove_ks($arr[$i]['short_name']);							
		...
		if(gmtime() < $arr[$i]['promote_end_date']){
			$arr[$i]['is_promoted'] = 1;
		}
        $arr[$i]['price'] = price_format($arr[$i]['shop_price']);
		$arr[$i]['price_nf'] = $arr[$i]['shop_price'];
    }
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                       sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
		$goods[$idx]['short_name'] = remove_ks($goods[$idx]['short_name']);							   
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        ...
        $goods[$idx]['short_style_name'] = add_style($goods[$idx]['short_name'], $row['goods_name_style']);
		$goods[$idx]['shop_price_nf']   = $row['shop_price'];
        $idx++;
    }
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		$scale = round(10/floatval($xe), 4);
		//$row['integral']      = $GLOBALS['_CFG']['integral_scale'] ? round($row['integral'] / $GLOBALS['_CFG']['integral_scale'],1) : 0;
		$row['integral']      = round($row['integral'] / $scale);
...
{
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc, img_order, img_show_in_thumbnail, img_show_in_detail' .
        ' FROM ' . $GLOBALS['ecs']->table('goods_gallery') .
        " WHERE goods_id = '$goods_id' order by img_order LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
 ...
 * @return  array
 */
function get_goods_main_img($goods_id)
{
    $sql = 'SELECT goods_img as img_url, goods_thumb as thumb_url, original_img as img_original' .
 ...
 * @return  array
 */
function get_goods_use_img($goods_id)
{
    $sql = 'SELECT use_img as img_url' .
        ' FROM ' . $GLOBALS['ecs']->table('goods') .
        " WHERE goods_id = '$goods_id'";
    $row = $GLOBALS['db']->getAll($sql);
    /* 格式化相册图片路径 */
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_weight, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.is_shipping ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['id']           = $row['goods_id'];
        $goods[$idx]['name']         = $row['goods_name'];/*
		if (strpos($row['goods_name'],'KIRKLAND SIGNATURE柯克兰') !== false){
			$goods[$idx]['name_no_ks']	 = str_replace('KIRKLAND SIGNATURE柯克兰','',$row['goods_name']);
		}
		if (strpos($row['goods_name'],'KIRKLAND柯克兰') !== false){
			$goods[$idx]['name_no_ks']	 = str_replace('KIRKLAND柯克兰','',$row['goods_name']);
		}
		if (strpos($row['goods_name'],'KIRKLAND柯蓝') !== false){
			$goods[$idx]['name_no_ks']	 = str_replace('KIRKLAND柯蓝','',$row['goods_name']);
		}*/
		$goods[$idx]['name']         = remove_ks($row['goods_name']);
        $goods[$idx]['brief']        = $row['goods_brief'];
        $goods[$idx]['market_price'] = price_format($row['market_price']);
		
		$no_eng_brand = remove_head_eng($row['goods_name']);	
				
		//preg_match("/[^a-zA-Z]\B.*/", $row['goods_name'], $no_eng_brand);
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                        ellipsis($no_eng_brand,$GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];						
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		
		$goods[$idx]['price_no_sign'] = $row['shop_price'];
		$goods[$idx]['goods_weight'] = $row['goods_weight']<1.0?$row['goods_weight']*1000 . 'g':round($row['goods_weight'],2) . 'kg';
		$goods[$idx]['index_short_name'] = ellipsis($no_eng_brand, 8);
		$goods[$idx]['index_short_name']   = remove_ks($goods[$idx]['index_short_name']);		
 ...
 * @return  array
 */
function assign_ext_cat_goods($cat_id, $num = 0, $order_rule = '')
{
	$sql = "SELECT * FROM ". $GLOBALS['ecs']->table("goods") ." WHERE goods_id IN (SELECT goods_id FROM ". $GLOBALS['ecs']->table("goods_cat") ." WHERE cat_id = " .$cat_id. ")";
	
	$order_rule = empty($order_rule) ? 'ORDER BY sort_order, goods_id DESC' : $order_rule;
    $sql .= $order_rule;
	if ($num > 0)
    {
        $sql .= ' LIMIT ' . $num;
 ...
 * @return  array
 */
function favourable_mix_info($act_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('favourable_activity_mix') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
        $row['start_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['start_time']);
        $row['end_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['end_time']);
        $row['formated_min_quantity'] = price_format($row['min_quantity']);
        $row['formated_max_quantity'] = price_format($row['max_quantity']);
        $row['gift'] = unserialize($row['gift']);
        if ($row['act_type'] == FAT_GOODS)
        {
            $row['act_type_ext'] = round($row['act_type_ext']);
