 ...
 * @return  array
 */
function assign_comment($id, $type, $page = 1, $rank = "", $show_img = false)
{			//$show_img = true;
	...
	$comment_like_limitation = 3;
	/*取得热门回复*/		
	$like_arr = comment_like_arr($id);
	$count_comment_like = count($like_arr);
	if($count_comment_like < $comment_like_limitation){$comment_like_limitation = $count_comment_like;}
	
	$rank_condition_count = " AND comment_rank >= 4";
	
	$all_comments_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
						" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0");
	
	$good_rank_count =  $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
						" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0". $rank_condition_count);
	
	$show_img_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM (SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
				"LEFT JOIN" . $GLOBALS['ecs']->table('comment_img') .
				"ON ". $GLOBALS['ecs']->table('comment').".comment_id = ".$GLOBALS['ecs']->table('comment_img') .".comment_id".
				" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0 AND ".$GLOBALS['ecs']->table('comment_img') .".comment_img_thumb_url <> '' GROUP BY ".$GLOBALS['ecs']->table('comment') .".comment_id) AS number");
				
	$count = $all_comments_count;		
	$all_comments_count = $comment_like_limitation + $all_comments_count;
	
	if ($rank != ""){
		$rank_condition = " AND comment_rank >= ". $rank;
		$show_img_condition = "";
		$count = $good_rank_count;
	}
	if ($show_img === true){
		$count = $show_img_count;
	}
	...
	if ($show_img === true){		
		...
		/*SELECT * 
		FROM ecs_comment
		LEFT JOIN ecs_comment_img ON ecs_comment.comment_id = ecs_comment_img.comment_id
		WHERE ecs_comment_img.comment_img_thumb_url <>  ''*/
		
		
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
				"LEFT JOIN" . $GLOBALS['ecs']->table('comment_img') .
				"ON ". $GLOBALS['ecs']->table('comment').".comment_id = ".$GLOBALS['ecs']->table('comment_img') .".comment_id".
				" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0 AND ".$GLOBALS['ecs']->table('comment_img') .".comment_img_thumb_url <> ''".
				" ORDER BY ".$GLOBALS['ecs']->table('comment_img') .".comment_id DESC";
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
            " WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0". $rank_condition.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['comment_id']]['parent_id']= $row['parent_id'];
        $arr[$row['comment_id']]['rank']     = $row['comment_rank'];
        $arr[$row['comment_id']]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
		$arr[$row['comment_id']]['user_id']  = $row['user_id'];
		$arr[$row['comment_id']]['is_admin']  = $row['is_admin'];
    }
    /* 取得已有回复的评论 */
    if ($ids)
    {
        $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
                " WHERE parent_id IN( $ids ) ORDER BY add_time";
        $res = $GLOBALS['db']->query($sql);
		$counter = 0;
        while ($row = $GLOBALS['db']->fetch_array($res))
        {     	$counter++;

				$arr[$row['parent_id']]['children'][$counter]['content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
				$arr[$row['parent_id']]['children'][$counter]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
				$arr[$row['parent_id']]['children'][$counter]['email']    = $row['email'];
				$arr[$row['parent_id']]['children'][$counter]['username'] = $row['user_name'];
				$arr[$row['parent_id']]['children'][$counter]['user_id']  = $row['user_id'];
				$arr[$row['parent_id']]['children'][$counter]['is_admin'] = $row['is_admin'];

        }
    }
	
	
	if ($rank == "" && $show_img === false){
			...
			if (!empty($like_arr)){
				...
				usort($like_arr, "sortByValue");
				$temp_arr = select_top_liked_comments($like_arr,$comment_like_limitation);
				$count_top = count($temp_arr);

				if ($page == 1){
					foreach ($arr as $key => $value){
						$temp_arr[] = $value;
					}
					$arr = $temp_arr;$temp_arr = array();
				}
			}

			$count = $comment_like_limitation + $count;
		}
    ...
    $pager['all_comments_record_count'] = $all_comments_count;
    $pager['good_rank_record_count'] = $good_rank_count;
    $pager['show_img_record_count'] = $show_img_count;
    $pager['page_count']   = $page_count;
...
{	
	...
	$prev_uri = $GLOBALS['ecs']->get_domain().$_SERVER['REQUEST_URI'];
	if (isset($_SESSION['language']) === false){
		...
		$language_a = '<select onChange="window.document.location.href=this.options[this.selectedIndex].value;" value="GO">
					   <option value="language.php?language=en&pre='.$prev_uri.'">English</option>
					   <option selected="selected">简体中文(Chinese)</option>
					   </select>';
	...
	else if ($language == '_en'){
		$language_a = '<select onChange="window.document.location.href=this.options[this.selectedIndex].value;" value="GO">
					   <option selected="selected">English</option>
					   <option value="language.php?language=ch&pre='.$prev_uri.'">简体中文(Chinese)</option>
					   </select>';
	}
...
function comment_like_arr($id){
	
	    $sql = 'SELECT comment_id FROM ' . $GLOBALS['ecs']->table('comment') .
				" WHERE id_value = '$id' AND comment_type = '$type' AND parent_id = 0".
				' ORDER BY comment_id DESC';
		$res = $GLOBALS['db']->getAll($sql);
		$like_arr = array();
