...
function orderSelectedResponse(result)
{
  ...
  try
  {
    ...
    if (result.payment != undefined)
    {
	  ...
	  var surplusObj2 = document.forms['theForm'].elements['surplus_cny'];
      if (surplusObj2 != undefined)
      {
        surplusObj2.disabled = result.pay_code == 'balance';
      }
    }
  }
  catch (ex) { }
...
function changeSurplus(val)
{
  ...
  var cny = document.forms['theForm'].elements['surplus_cny'].value;
  Ajax.call('flow.php?step=change_surplus', 'surplus=' + val + '&surplus_cny=' + cny, changeSurplusResponse, 'GET', 'JSON');
}

/* *
 * 改变人民币余额
 */
function changeSurplus2(val)
{
  if (selectedSurplus == val)
  {
    return;
  ...
  var cad = document.forms['theForm'].elements['surplus'].value;
  Ajax.call('flow.php?step=change_surplus', 'surplus=' + cad + '&surplus_cny=' + val, changeSurplusResponse2, 'GET', 'JSON');
}
...
//人民币
function changeSurplusResponse2(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById("ECS_SURPLUS_CNY_NOTICE").innerHTML = obj.error;
      document.getElementById('ECS_SURPLUS_CNY').value = '0';
      document.getElementById('ECS_SURPLUS_CNY').focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById("ECS_SURPLUS_CNY_NOTICE").innerHTML = '';
    }
    catch (ex) { }
    orderSelectedResponse(obj.content);
  }
...
function checkConsignee(frm)
{
  ...
  if (frm.elements['country'].value == 1 && Utils.isEmpty(frm.elements['consignee_id'].value))
  {
    msg.push("由于收货人在中国，为保证包裹顺利清关，请填写收货人身份证号码。");
    err = true;
  }
  
  if (! Utils.isEmpty(frm.elements['consignee_id'].value) && ! Utils.isID(frm.elements['consignee_id'].value)){
  	msg.push("请输入有效的身份证号码。");
    err = true;
  }
  ...
  else if (! Utils.isEmpty(frm.elements['consignee_id'].value) && (frm.elements['have_id_front'].value == 0 || frm.elements['have_id_back'].value == 0)){
  	alert("记得上传身份证正面与反面的扫描件哦~");
  }
