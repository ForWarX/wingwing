...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
	...
	if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
    {
        ...
        {
            $order['order_amount'] = $order['order_amount'] + $total['surplus_cny'];
            $order['surplus_cny'] = 0;
        }
        ...
        {
			$order['surplus_cny'] = round($order['order_amount']*$xe,1);
	...
	if ($order['user_id'] > 0 && $order['surplus_cny'] > 0)
    {
		showr($order);
		showr($total);
        log_account_change($order['user_id'], 1, $total['surplus_cny'] * (-1), 0, 0, 0, sprintf($_LANG['pay_order_cny'], $order['order_sn']));
    }
