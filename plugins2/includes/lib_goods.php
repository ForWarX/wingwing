 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $image_order='',$image_show_in_thumbnail='',$image_show_in_detail='')
{
	if (empty($image_order) === true){
				$image_order = array();
	}
	if (empty($image_show_in_thumbnail) === true){
				$image_show_in_thumbnail = array();
	}
	if (empty($image_show_in_detail) === true){
				$image_show_in_detail = array();
	}
	//showr($image_order);
    /* 是否处理缩略图 */
    $proc_thumb = (isset($GLOBALS['shop_id']) && $GLOBALS['shop_id'] > 0)? false : true;
	//showr($image_show_in_thumbnail);
	//showr($image_descs);
    ...
    { 
        ...
        if ($flag)
        {
			...
			$img_order = $image_order[$key];
			$img_show_in_detail = $image_show_in_detail[$key];
			$img_show_in_thumbnail = $image_show_in_thumbnail[$key];
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, img_order, img_show_in_thumbnail, img_show_in_detail) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original', '$img_order', '$img_show_in_thumbnail', '$img_show_in_detail')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
 ...
 * @return  void
 */
function handle_func($goods_id, $function_order, $function_content, $function_content_en,$count_old_func)
{
	foreach ($function_content AS $key => $func_content)
    { 
        /* 是否成功有效 */
        $flag = false;
        if ((isset($func_content) && $func_content != "") || isset($function_content_en[$key]) && $function_content_en[$key] != "")
        {   
            $flag = true;
        }
        ...
        if ($flag)
        {
			$func_title = '0';
			$func_title_en = '0';
			$func_content = $function_content[$key];
			$func_content_en = $function_content_en[$key];
			$func_order = $key+1+$count_old_func;
			

            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_func') . " (goods_id, goods_func_title, goods_func_title_en, goods_func_content, goods_func_content_en, goods_func_order) " .
                    "VALUES ('$goods_id', '$func_title', '$func_title_en', '$func_content', '$func_content_en', '$func_order')";
            $GLOBALS['db']->query($sql);
        }
    }
}
 ...
 * @return  void
 */
function handle_use($goods_id, $using_order, $using_title, $using_title_en, $using_content, $using_content_en)
{
    foreach ($using_content AS $key => $use_content)
    { 
        /* 是否成功有效 */
        $flag = false;
        if (((isset($use_content) && $use_content != "") || (isset($using_content_en[$key]) && $using_content_en[$key] != "")) || ((isset($use_title) && $use_title != "") || (isset($using_title_en[$key]) && $using_title_en[$key] != "")))
        {   
            $flag = true;
        }
        ...
        if ($flag)
        {
			$use_title = $using_title[$key];
			$use_title_en = $using_title_en[$key];
			$use_content = $using_content[$key];
			$use_content_en = $using_content_en[$key];
			$use_order = $using_order[$key];
			
			//showr($image_show_in_thumbnail);

            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_use') . " (goods_id, goods_use_title, goods_use_title_en, goods_use_content, goods_use_content_en, goods_use_order) " .
                    "VALUES ('$goods_id', '$use_title', '$use_title_en', '$use_content', '$use_content_en', '$use_order')";
            $GLOBALS['db']->query($sql);
        }
    }
}
