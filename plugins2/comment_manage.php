...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取评论详细信息并进行字符处理 */
    $sql = "SELECT * FROM " .$ecs->table('comment'). " WHERE comment_id = '$_REQUEST[id]' ORDER BY add_time";
    ...
    $reply_info = $db->getAll($sql);
	foreach ($reply_info as $k => $v){
		$reply_info[$k]['add_time'] = local_date($_CFG['time_format'], $v['add_time']);
	}
    ...
    {*/
        ...
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
               ...
               "VALUES('$_POST[comment_type]', '$_POST[id_value]','$_POST[email]', " .
                    "'$_SESSION[admin_name]','$_POST[content]','" . gmtime() . "', '$ip', '0', '$_POST[parent_id]', '1')";
    //}
    $db->query($sql);
...
//-- 删除某一条评论Ajax
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'remove_ajax')
{
	$json   = new JSON;
	$result = array('error' => 0, 'message' => '', 'result' => '0', 'id' => $_REQUEST['ajax_comment_id']);

    check_authz_json('comment_priv');

    $ajax_comment_id = intval($_REQUEST['ajax_comment_id']);

    $sql = "DELETE FROM " .$ecs->table('comment'). " WHERE comment_id = '$ajax_comment_id'";
    $res = $db->query($sql);
    if ($res)
    {
        $db->query("DELETE FROM " .$ecs->table('comment'). " WHERE parent_id = '$ajax_comment_id'");
		$result['result'] = 1;
    }
