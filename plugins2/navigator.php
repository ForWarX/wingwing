    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,name_cht,name_en,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('$item_name','$item_cht_name','$item_en_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type')";
            }
        }

        if(empty($sql))
        {
             $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,name_cht,name_en,ifshow,vieworder,opennew,url,type) VALUES('$item_name','$item_cht_name','$item_en_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type')";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    {
        $item_name = $_REQUEST['item_name'];
        $item_cht_name = $_REQUEST['item_cht_name'];
        $item_en_name = $_REQUEST['item_en_name'];		
        $item_url = $_REQUEST['item_url'];
        $item_ifshow = $_REQUEST['item_ifshow'];
        $item_opennew = $_REQUEST['item_opennew'];
        $item_type = $_REQUEST['item_type'];
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',name_cht='$item_cht_name',name_en='$item_en_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type' WHERE id='$id'";
        }
        else
        {
            //目标不是分类
