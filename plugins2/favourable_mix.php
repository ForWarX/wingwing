...
$exc = new exchange($ecs->table('favourable_activity_mix'), $db, 'act_id', 'act_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href' => 'favourable_mix.php?act=add', 'text' => $_LANG['add_favourable']));
    ...
    /* 显示商品列表页面 */
    assign_query_info();
    $smarty->display('favourable_mix_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('favourable_mix_list.htm'), '',
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $id = intval($_GET['id']);
    $favourable = favourable_mix_info($id);
    if (empty($favourable))
    {
    ...
    $url = 'favourable_mix.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            /* 删除记录 */
            $sql = "DELETE FROM " . $ecs->table('favourable_activity_mix') .
            ...
            $links[] = array('text' => $_LANG['back_favourable_list'], 'href' => 'favourable_mix.php?act=list&' . list_link_postfix());
...
elseif ($_REQUEST['act'] == 'edit_sort_order')
{
    ...
    $sql = "UPDATE " . $ecs->table('favourable_activity_mix') .
            " SET sort_order = '$val'" .
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    $href = 'favourable_mix.php?act=list';
    if (!$is_add)
    {
    ...
    assign_query_info();
    $smarty->display('favourable_mix_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 检查金额上下限 */
    $min_quantity = floatval($_POST['min_quantity']) >= 0 ? floatval($_POST['min_quantity']) : 0;
    $max_quantity = floatval($_POST['max_quantity']) >= 0 ? floatval($_POST['max_quantity']) : 0;
    if ($max_quantity > 0 && $min_quantity > $max_quantity)
    {
    ...
    $favourable = array(
        ...
        'max_quantity'    => floatval($_POST['max_quantity']),
    ...
    if ($is_add)
    {
        $db->autoExecute($ecs->table('favourable_activity_mix'), $favourable, 'INSERT');
        $favourable['act_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($ecs->table('favourable_activity_mix'), $favourable, 'UPDATE', "act_id = '$favourable[act_id]'");
    }
    ...
    {
        $links = array(
            array('href' => 'favourable_mix.php?act=add', 'text' => $_LANG['continue_add_favourable']),
            array('href' => 'favourable_mix.php?act=list', 'text' => $_LANG['back_favourable_list'])
        );
    ...
    {
        $links = array(
            array('href' => 'favourable_mix.php?act=list&' . list_link_postfix(), 'text' => $_LANG['back_favourable_list'])
        );
...
function favourable_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('favourable_activity_mix') .
                " WHERE 1 $where";
        ...
        /* 查询 */
        $sql = "SELECT * ".
                "FROM " . $GLOBALS['ecs']->table('favourable_activity_mix') .
                " WHERE 1 $where ".
