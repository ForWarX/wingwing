...
'account_deposit', 'pcoin_deposit', 'account_log', 'account_detail', 'act_account', 'act_pcoin_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
if ($action == 'default')
{
	...
	$expire = local_getdate($info['vip_end']);
	$expire_date = $expire['year'] . "-" . $expire['mon'] . "-" . $expire['mday'];
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register2($username, $password, $email, $other) !== false)
        {
            /*把新注册用户的扩展信息插入数据库*/
...
elseif ($action == 'order_detail')
{
    ...
    if ($order['order_amount'] > 0)
    {
        if ($order['order_status'] == OS_UNCONFIRMED || $order['order_status'] == OS_CONFIRMED)
        {
            ...
            if ($user['user_money'] + $user['credit_line'] > 0)
            {
                ...
                $smarty->assign('max_surplus', '$' . $user['user_money']);
            }
			if ($user['user_money_cny'] + $user['credit_line'] > 0)
            {
                $smarty->assign('allow_edit_surplus', 1);
                $smarty->assign('max_surplus_cny', '￥' . $user['user_money_cny']);
            }
        }
    }
	...
	foreach ($goods_list as $key => $value){
		$result = is_user_comment($user_id,$value['goods_id']);
		if ($result['result'] === true){
			$goods_list[$key]['is_comment'] = 1;
			$goods_list[$key]['comment_id'] = $result['comment_id'];
		}
		else{$goods_list[$key]['is_comment'] = 0;}
	}
...
elseif ($action == 'act_edit_address')
{
    ...
    $address = array(
		...
		'consignee_id'  => isset($_POST['consignee_id']) ? compile_str(make_semiangle(trim($_POST['consignee_id']))) : '',
	...
	/*身份证扫描件处理*/
	if (isset($_FILES['id_front']['error'])) // php 4.2 版本才支持 error
    {
		$tmp_res = upload_id($_FILES['id_front'], 'front', $address, $image);
		if ($tmp_res[0]){
			$address['have_id_front'] = 1;
		}
	}
	if (isset($_FILES['id_back']['error'])) // php 4.2 版本才支持 error
	{
		$tmp_res = upload_id($_FILES['id_back'], 'back' , $address, $image);
		if ($tmp_res[0]){
			$address['have_id_back'] = 1;
		}
	}
...
elseif ($action == 'comment_list')
{
	...
	//showr(get_comment_list($user_id, $pager['size'], $pager['start']));
...
elseif ($action == 'pcoin_deposit')
{
    ...
    $surplus_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    $account    = get_surplus_info($surplus_id);
	if ($rank['rank_name'] == "高级会员"){
		$deposit_bonus = 5;	
	}
	else{
		$deposit_bonus = 0;	
	}
	$smarty->assign('deposit_bonus', $deposit_bonus);
    $smarty->assign('payment', get_online_payment_list(false));
    $smarty->assign('order',   $account);
    $smarty->display('user_transaction.dwt');
}
...
/* 对会员积分充值的处理 */
elseif ($action == 'act_pcoin_account'){
	include_once(ROOT_PATH . 'includes/lib_clips.php');
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $amount = isset($_POST['amount']) ? intval($_POST['amount']) : 0;
    if ($amount % 20 != 0)
    {
        show_message("您好，请输入20倍数的充值金额。如：20，40，60……");
    }
	...
	if ($rank['rank_name'] == "高级会员"){
		$deposit_bonus = $amount / 20;	
	}
	else{
		$deposit_bonus = 0;	
	}
	include_once(ROOT_PATH . 'languages/zh_cn/common.php');
	$cad_amount = round($amount/floatval($xe),2);
	/* 变量初始化 */
    $surplus = array(
            'user_id'      => $user_id,
            ...
            'amount'       => $cad_amount,
			'pay_points'   => $amount * 10 + $deposit_bonus
    ...
    include_once(ROOT_PATH .'includes/lib_payment.php');
      ...
      //计算此次预付款需要支付的总金额
      $order['order_amount']   = $cad_amount + $payment_info['pay_fee'];
      ...
      $smarty->assign('cad_amount',  price_format($cad_amount, false));
	  $smarty->assign('amount',  $amount, false);
	  $smarty->assign('pamount',  $amount * 10);
      $smarty->assign('order',   $order);
      $smarty->display('user_transaction.dwt');//调需要的模版，也可以在user_transaction.dwt中操作，模版信息就复制act_account的模版直接使用，在其中可以添加一行说明充值了多少积分，在这就不说明了，如果有需要的朋友可以给我留言。
}
...
elseif ($action == 'act_account')
{
	...
	$currency_type = isset($_POST['currency']) ? intval($_POST['currency']) : 0;
	if ($currency_type == 1){
		$xe_rate = get_exchange_rate('CNY','CAD');
		$amount = isset($_POST['amount']) ? round(floatval($_POST['amount']/$xe_rate),2) : 0;
	}
	else{
    ...
    else
    {
		...
		if($surplus['currency_type'] > 0){
			$smarty->assign('amount_in_cny',  ' (约合人民币￥' . strval($_POST['amount']) . ')');
		}
...
elseif ($action == 'act_edit_surplus')
{
    ...
    $surplus_cny = floatval($_POST['surplus_cny']);
    if ($surplus <= 0 && $surplus_cny <= 0)
    {
	...
	$surplus_cny_cvt = round($surplus_cny/$xe,2);
	if ($surplus_cny_cvt > $order['order_amount'])
    {
        $surplus_cny_cvt = $order['order_amount'];
		$surplus_cny = round($order['order_amount']*floatval($xe),1);
    }
	...
	if ($surplus_cny > $user['user_money_cny'] + $user['credit_line'])
    {
        $err->add("人民币余额不足");
        $err->show($_LANG['order_detail'], 'user.php?act=order_detail&order_id=' . $order_id);
    }
	...
	$order['surplus_cny'] += $surplus_cny;
	$order['order_amount'] -= $surplus;
    $order['order_amount'] -= $surplus_cny_cvt;
    if ($order['order_amount'] > 0)
    {
        $cod_fee = 0;
	...
	$change_desc2 = sprintf($_LANG['pay_order_by_surplus_cny'], $order['order_sn']);
    log_account_change($user['user_id'], 0, (-1) * $surplus, 0, 0, 0, $change_desc);
	log_account_change($user['user_id'], 1, (-1) * $surplus_cny, 0, 0, 0, $change_desc2);
...
elseif ($action == 'act_transform_points')
{
    ...
    switch ($rule_key)
    {
        case TO_P :
            ...
            log_account_change($row['user_id'], 0, 0, 0, 0, $result_points, $_LANG['transform_points'], ACT_OTHER);
        ...
        case TO_R :
            $result_points = floor($num * $to / $from);
            $user->set_points($row['user_name'], array($bbs_key=>0 - $num)); //调整论坛积分
            log_account_change($row['user_id'], 0, 0, 0, $result_points, 0, $_LANG['transform_points'], ACT_OTHER);
        ...
        case FROM_P :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0, 0, 0-$num, $_LANG['transform_points'], ACT_OTHER); //调整商城积分
        ...
        case FROM_R :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0, 0-$num, 0, $_LANG['transform_points'], ACT_OTHER); //调整商城积分
            $user->set_points($row['user_name'], array($bbs_key=>$result_points)); //调整论坛积分
