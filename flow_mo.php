...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    /* 更新：如果是一步购物，先清空购物车 */
    if ($_CFG['one_step_buy'] == '1' || $goods->xpbuy)
    {
        clear_cart();
    }
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
            ...
            $result['content'] = insert_cart_info();
            $result['one_step_buy'] = $_CFG['one_step_buy'] == '0'? intval($goods->xpbuy): $_CFG['one_step_buy'];
			$_SESSION['xpbuy'] = $result['one_step_buy'];
        }
        else
        {
            $result['message']  = $err->last_message();
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            if (register2(trim($_POST['username']), trim($_POST['password']), trim($_POST['email'])))
            {
                /* 用户注册成功 */
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
			...
			'consignee_id'  => empty($_POST['consignee_id']) ? '' :   compile_str(make_semiangle(trim($_POST['consignee_id']))),
			...
			'have_id_front'     => empty($_POST['have_id_front'])  ? 0 : $_POST['have_id_front'],
			'have_id_back'     => empty($_POST['have_id_back'])  ? 0 : $_POST['have_id_back']
        );
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    {
		$is_multi_discount = is_multi_discount();
        $discount = compute_discount($is_multi_discount);
		$discount2 = compute_discount_with_favourable_list_mix($is_multi_discount);
		$discount_number = $discount['discount'] + $discount2['discount'];
        $smarty->assign('discount', $discount_number);
        $favour_name = empty($discount['name']) ? '' : join(',', $discount['name']);
		if (!empty($favour_name)){$favour_name .= '<font style="color:#000;">，</font>'; }
		if (!empty($discount2['name'])){ $favour_name .= join(',', $discount2['name'] );}
        $smarty->assign('your_discount', sprintf($_LANG['your_discount'], $favour_name, price_format($discount_number)));
    }
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
	...
	$surplus_for_judge = floatval($_GET['surplus']);
	$money_col = 'user_money';
    
	$surplus_cny = round(floatval($_GET['surplus_cny'])/get_exchange_rate('CNY','CAD'),2);
	$surplus_cny_for_judge = floatval($_GET['surplus_cny']);
	$money_cny_col = 'user_money_cny';
    	
	$user_info = user_info($_SESSION['user_id']);

    if ($user_info[$money_col] + $user_info['credit_line'] < $surplus_for_judge)
    {
        $result['error'] = $_LANG['surplus_not_enough'];
    }
	else if($user_info[$money_cny_col] + $user_info['credit_line'] < $surplus_cny_for_judge){
		$result['error'] = $_LANG['surplus_not_enough'];
	}
    else
    {
        ...
        else
        {
			...
			$order['surplus_in_cny'] = floatval($_GET['surplus_cny']);
            /* 计算订单的费用 */
            $total = order_fee($order, $cart_goods, $consignee);
            $smarty->assign('total', $total);
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
	...
	$order['surplus_cny']  = round($order['surplus_cny']/$xe,2);
	...
	$order['surplus_cny']  = round($total['surplus_cny']*$xe,1);
    $order['tax']          = $total['tax'];
    ...
    {
        log_account_change($order['user_id'], 1, $order['surplus_cny'] * (-1), 0, 0, 0, sprintf($_LANG['pay_order_cny'], $order['order_sn']));
    }
...
else
{
    ...
    /* 如果是一步购物，跳到结算中心 */
    if ($_CFG['one_step_buy'] == '1' || $_SESSION['xpbuy'])
    {
        ecs_header("Location: flow.php?step=checkout\n");
        exit;
    }
    ...
    $smarty->assign('discount', $discount_number);
    $favour_name = empty($discount['name']) ? '' : join(',', $discount['name'] );
	if (!empty($discount2['name'])){ $favour_name .= '<font style="color:#000;">，</font>' . join(',', $discount2['name'] );}
    //$favour_name .= empty($discount2['name']) ? '' : join(',', $discount2['name'] );
    $smarty->assign('your_discount', sprintf($_LANG['your_discount'], $favour_name, price_format($discount_number)));
